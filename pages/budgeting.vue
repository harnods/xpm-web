...
<template>
  ...
  <Teleport to="#layout-header-actions">
    <MpButton variant="primary" size="md">Set budget</MpButton>
  </Teleport>
  ...
  <MpFlex direction="column" gap="4" width="full" style="min-width:0;">
    ...
    <!-- 1) Tabs -->
    <div :class="tabBar">
      <button type="button" :class="tabOn">Utilization</button>
      <button type="button" :class="tab">Configured budgets</button>
    </div>
        ...
        <tbody>
          <tr v-for="row in rows" :key="row.key" :class="tr">
            ...
            <td :class="tdNum">
              ...
              <span v-else :class="setLink">{{ row.right.link }}</span>
            </td>
