...
// ─── Mode ─────────────────────────────────────────────────────────
// create        → requester filing a new claim (everything editable)
// requester-edit→ requester editing own claim (only attachments, descriptions
//                 and Additional details editable; items view-only)
// approver-edit → approver editing a claim from Transactions (everything
//                 editable EXCEPT Category and each item's Subcategory)
const props = withDefaults(defineProps<{
  mode?: 'create' | 'requester-edit' | 'approver-edit'
}>(), { mode: 'create' })

const lockCategory    = computed(() => props.mode !== 'create')
const lockSubcategory = computed(() => props.mode !== 'create')
const isRequesterEdit = computed(() => props.mode === 'requester-edit')
...
// ─── Edit prefill ─────────────────────────────────────────────────
// In edit modes the claim is pre-selected from the ?category=<slug>
// query — setting categoryId triggers watch(categoryId) → fillSampleData,
// which populates the whole form just like the requester last saved it.
const route = useRoute()
if (isEdit.value && typeof route.query.category === 'string') {
...
// Devtools (showcase only — stripped from production): force the bank
// account list empty / force the required-field error state.
const isDev           = import.meta.dev
const devOpen         = ref(false)
const devNoBankAccount = ref(false)
const devBankError    = ref(false)

const filteredBankAccounts  = computed(() =>
  devNoBankAccount.value ? [] : MOCK_BANK_ACCOUNTS.filter(b => b.currency === displayCurrency.value)
)
// Invalid only while nothing is selected — picking an account clears the error.
const bankAccountInvalid = computed(() => devBankError.value && !selectedBankAccountId.value)
...
function saveBankAccount() {
  ...
  devNoBankAccount.value = false   // a real account now exists — leave the showcase empty state
...
// immediate: in edit modes categoryId is pre-set, so seed the form on load
}, { immediate: true })
...
// Claim id comes from the route param on edit routes (/.../[id]/edit)
const claimId = computed(() => String(route.params.id ?? route.query.id ?? ''))
// Capitalised claim type for toast copy: "Reimbursement" / "Cash advance"
const claimTypeLabel = computed(() => (route.query.type === 'cash-advance' ? 'Cash advance' : 'Reimbursement'))
// Where Cancel / Save returns to, per mode (the claim detail the edit was opened from)
const returnTo = computed(() =>
  props.mode === 'approver-edit'
    ? `/transactions/claims/${claimId.value}`
    : '/my-claims',
)
const submitLabel = computed(() => (isEdit.value ? 'Save changes' : 'Submit'))
function cancel() { router.push(returnTo.value) }
async function submit() {
  submitting.value = true
  await new Promise(r => setTimeout(r, 600))
  submitting.value = false
  const title = isEdit.value
    ? `${claimTypeLabel.value} changes saved`
...
const devFab = css({
  ...
  boxShadow: 'var(--shadows-lg)', _hover: { opacity: '0.9' },
})
const devPanel = css({
...
<template>
    ...
    <MpFlex direction="column" gap="5" :style="{ marginBottom: hasSubcategories ? 'var(--mp-spacing-8)' : 'var(--mp-spacing-5)' }">
      ...
      <MpFormControl id="claim-category" isRequired :class="field">
        ...
        <MpAutocomplete
          ...
          is-searchable
          :is-clearable="!lockCategory"
          :is-disabled="lockCategory"
          use-portal
        />
        <MpFormHelpText v-if="lockCategory">Category can't be changed when editing this claim.</MpFormHelpText>
      </MpFormControl>
      ...
      <MpFormControl id="transaction-date" isRequired :class="field">
        ...
        <MpDatePicker
          ...
          :use-portal="true"
          :is-disabled="isRequesterEdit"
        />
      </MpFormControl>
    </MpFlex>
  </div>

  <!-- ═════ Variant B — Reimbursement item table (full width) ═════ -->
  <MpFlex v-if="hasSubcategories" direction="column" gap="4" marginBottom="8">
            ...
            <tbody>
              <tr v-for="(it, i) in items" :key="i">
                ...
                <td :class="td">
                  ...
                  <MpText v-else size="body" weight="regular" color="gray.600">No attachment</MpText>
                </td>
                ...
                <td :class="tdRight">
                  <MpFlex align="center" gap="1" justify="flex-end">
                    ...
                    <MpButton v-if="!isRequesterEdit" variant="ghost" size="sm" left-icon="edit" aria-label="Edit item" @click="openItem(i, false)" />
                    <MpButton v-if="!isRequesterEdit" variant="ghost" size="sm" left-icon="minus-circular" aria-label="Remove item" @click="removeItem(i)" />
                  </MpFlex>
                </td>
              </tr>
              <!-- Total row -->
        ...
        <MpButton v-if="!isRequesterEdit" variant="secondary" size="md" left-icon="add" style="align-self:flex-start;" @click="openAddItem">
          Add item
        </MpButton>
        ...
        <template v-if="isInternational">
          <MpFlex direction="column" gap="4">
            ...
            <MpFormControl id="bank-account-b" isRequired :is-invalid="bankAccountInvalid" :class="field">
              ...
              <MpAutocomplete
                ...
                empty-text="No bank account available"
                is-searchable
                is-show-button-action
                is-clearable
                :is-disabled="isRequesterEdit"
                use-portal
                :content-attrs="{ class: 'bank-ac-empty' }"
              ...
              <MpFormErrorMessage>You must select bank account</MpFormErrorMessage>
            </MpFormControl>
          </MpFlex>
        </template>
  </MpFlex>
  ...
  <div :class="page">
    ...
    <MpFlex v-if="!hasSubcategories" direction="column" gap="5" marginBottom="8">
        <!-- Amount -->
            ...
            <MpPopover id="amount-currency-popover" is-close-on-select use-portal is-adaptive-width placement="bottom-start">
              <MpPopoverTrigger>
                <MpInputGroup style="width:100%;">
                  ...
                  <MpInput :modelValue="amount" placeholder="0" :isFullWidth="true" :is-disabled="isRequesterEdit"
                    @click.stop
        ...
        <MpFormControl id="vendor" isRequired :class="field">
          ...
          <MpInput id="input-vendor" v-model="vendor" placeholder="Enter vendor" :isFullWidth="true" :is-disabled="isRequesterEdit" />
        </MpFormControl>
        ...
        <template v-if="isInternational">
          <MpFlex direction="column" gap="4">
            ...
            <MpFormControl id="bank-account" isRequired :is-invalid="bankAccountInvalid" :class="field">
              <MpFormLabel>Bank account</MpFormLabel>
              <MpAutocomplete
  ...
  <div :class="stickyFooter">
    ...
    <MpButton variant="primary" size="md" :isLoading="submitting" @click="submit">{{ submitLabel }}</MpButton>
  </div>
        ...
        <MpFlex direction="column" gap="5">
          ...
          <MpBanner id="banner-wtm" variant="info" is-inline>
            ...
            <MpBannerDescription id="banner-description-wtm">
              Ensure your bank account details are valid to avoid payment failures.
            </MpBannerDescription>
          </MpBanner>
      ...
      <MpDrawerBody>
        <MpFlex direction="column" gap="5">
          ...
          <MpFormControl id="draft-subcategory" isRequired>
            ...
            <MpSelect id="select-subcategory" v-model="draft.subcategory" placeholder="Select subcategory" :is-full-width="true" :is-disabled="drawerReadOnly || lockSubcategory">
  ...
  <!-- ═════ Floating devtools (dev only) — showcase bank account empty state ═════ -->
  <Teleport to="body">
    <template v-if="isDev">
      <!-- Panel -->
      <div v-if="devOpen" :class="devPanel">
        <MpFlex direction="column" gap="3">
          <MpFlex align="center" justify="space-between">
            <MpText weight="semiBold" color="dark" style="font-size:13px;">Devtools · Bank account</MpText>
            <button :class="devClose" aria-label="Close devtools" @click="devOpen = false">✕</button>
          </MpFlex>

          <button :class="devNoBankAccount ? devToggleOn : devToggleOff" @click="devNoBankAccount = !devNoBankAccount">
            {{ devNoBankAccount ? 'Showing empty state' : 'Show empty state' }}
          </button>

          <MpText color="gray.600" style="font-size:11px; line-height:16px;">
            Forces the bank account list empty to reveal the blank slate. Only visible on international categories.
          </MpText>

          <button :class="devBankError ? devToggleOn : devToggleOff" @click="devBankError = !devBankError">
            {{ devBankError ? 'Showing error state' : 'Show error state' }}
          </button>

          <MpText color="gray.600" style="font-size:11px; line-height:16px;">
            Shows the required-field error on the bank account selection (as if submitted without selecting). Clears once an account is picked.
          </MpText>
        </MpFlex>
      </div>
...
<!--
  Bank account dropdown empty state — rendered inside the autocomplete list.
  The autocomplete portals its content to <body>, so this style is global (not scoped)
  and namespaced by the `bank-ac-empty` class passed via :content-attrs.
  Layout: illustration (::before) → title (empty-text prop) → description (::after).
-->
<style>
.bank-ac-empty .mp-autocomplete__emptyText {
  display: flex;
  ...
  --mp-text-color: var(--mp-colors-dark);
  color: var(--mp-colors-dark);
  font-weight: 600;
  font-size: 14px;
  line-height: 20px;
}
.bank-ac-empty .mp-autocomplete__emptyText::before {
  content: "";
  width: 140px;
  height: 116px;
  margin-bottom: 4px;
  background: url('/images/empty-no-data.png') center / contain no-repeat;
}
.bank-ac-empty .mp-autocomplete__emptyText::after {
  content: "You haven't created any bank account yet";
  font-weight: 400;
  font-size: 13px;
  line-height: 18px;
  color: var(--mp-colors-gray-600);
}
</style>
